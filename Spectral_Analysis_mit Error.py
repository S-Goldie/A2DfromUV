...
#I would like a more elegant way to handle this error during the data set loading, but for now this works. If the incorrect material has been chosed and the required wavelengths have not been measured, this sets to the closest possible.
#However, issue with this method is that the smoothing will be very wrong when running right up against the edge of usable data points.
if lf_lim < 0:
    lf_lim = 0
if rt_lim < 0:
...
#Lists to contain output values from each spectrum
final_wavelength_a = []
final_error_a = []
final_window_list = []
final_fraction_list = []
final_flake_length = []
lw_filtered = np.zeros([len(spectra),len(spectra[0]),len(spectra[0])])            #used to save final processed spectra for illustrative plotting
sec_diff = np.zeros([len(spectra),len(spectra[0]),len(spectra[0])])             #array of differentiated data, [i,j,x] where i is the spectrum index, j is the smoothness index, x is the wavelength index
interpol_array = np.zeros([len(spectra),len(spectra[0]),1000])                  #array for interpolated data storage, [i,j,x] where i is the spectrum index, j is the smoothness index, x is the interpolated wavelength index
x_new=np.linspace(wavelength[lf_lim+1],wavelength[rt_lim-1],1000)

processed_spectra = np.zeros([int(1+2*len(spectra)), int(len(wavelength))])           #empty array to save the exciton wavelength and optimal window for each gamma value
processed_spectra[0] = wavelength

for i in range(len(spectra)):       #Iterate over each spectra in turn for analysis
    print(i+1)
    ...
    try:
        while convergence_flag != True:
            ...
            if (x_new[rightzero[k]] - x_new[leftzero[k]]) > 10:     #a peak narrower than 15nm is assumed to be noise crossing the x-axis
                if x_new[rightzero[k]] - x_new[diff_min[k]] > 5 and x_new[diff_min[k]] - x_new[leftzero[k]] > 5 :   #the intercepts must be either side and ruther than 5nm from the minimal point. (Using the minimum point like this can cause errors with spectra that have discontinuities at localised points due to source or detector changes)
                    if right_smoothness <= 0.02 and left_smoothness <= 0.02 and center_smoothness <= 0.02:      #all metrics must show less than 2.5% variation within a sub-set size defined from the data interval.
...
plt.figure("Spectral_Analyis Smoothness")
...
for i in range(len(spectra)):
    ...
    plt.plot(x_new,interpol_array[i,int((final_window_list[i]/2)-1.5),:],color=colors[i], linestyle=':', label=(labels[i] + ' interpolated'))
    plt.axhline(0)
    if final_wavelength_a[i] != 0:
        plt.axvline(final_wavelength_a[i],color=colors[i])
...
output_headings = str('Sample,Exciton Wavelength / nm,Error / nm,Exciton Energy / eV,Error / eV,<N>vf,Flake Length / nm,Smoothing Window,Smoothing Fraction')
try:
    np.savetxt(Filename.removesuffix('.csv') + '_Metrics.csv', np.column_stack((names, final_wavelength_a, final_error_a, final_energy_a, final_energy_a_error, final_thickness, final_flake_length, final_window_list, final_fraction_list)), delimiter=',', header=output_headings, fmt='%s')
except AttributeError:
    np.savetxt(Filename[:-4] + '_Metrics.csv', np.column_stack((names, final_wavelength_a, final_error_a, final_energy_a, final_energy_a_error, final_thickness, final_flake_length, final_window_list, final_fraction_list)), delimiter=',', header=output_headings, fmt='%s')
